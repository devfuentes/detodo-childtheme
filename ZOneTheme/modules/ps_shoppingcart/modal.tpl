              ...
              <div class="cart-prices px-3">
                  ...
                  {foreach from=$cart.subtotals item="subtotal"}
                    {if $subtotal.value && $subtotal.type !== 'tax'}
                      <div class="cart-summary-line cart-subtotal-{$subtotal.type}">
                        ...
                        {else}
                          <span class="price">{$subtotal.value} {if 'shipping' == $subtotal.type}{hook h='displayCheckoutSubtotalDetails' subtotal=$cart.subtotals.shipping}{/if}</span>
                        {/if}
                      </div>
                    {/if}
                  {/foreach}
                ...
                <div class="cart-total">
                  ...
                  {if $cart.subtotals.tax}
                    <div class="cart-summary-line product-tax">
                      <small class="label">{l s='%label%:' sprintf=['%label%' => $cart.subtotals.tax.label] d='Shop.Theme.Global'}</small>
                      <small class="value">{$cart.subtotals.tax.value}</span>
                    </div>
                  {/if}
                </div>
              </div>
