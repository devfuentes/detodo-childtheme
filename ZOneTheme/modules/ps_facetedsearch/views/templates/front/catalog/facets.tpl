      ...
      <section class="facet clearfix">
        ...
        {if in_array($facet.widgetType, ['radio', 'checkbox'])}
          {block name='facet_item_other'}
            <ul id="facet_{$_expand_id}" class="faceted-{$facet.widgetType} collapse{if !$_collapse} show{/if}">
                  ...
                  <div class="facet-label custom-checkbox-wrapper {if $filter.active} active {/if}">
                    {if $facet.multipleSelectionAllowed}
                      <span class="custom-checkbox">
                        ...
                        {else}
                          <span {if !$js_enabled} class="ps-shown-by-js" {/if}><i class="material-icons check-icon">check</i></span>
                        {/if}
                      </span>
                    {else}
                    ...
                    <a
                      ...
                      rel="nofollow"
                    >
                      {$filter.label}{if $filter.magnitude and $show_quantities}<span class="magnitude">({$filter.magnitude})</span>{/if}
                    </a>
                  </div>
                </li>
              {/foreach}
        ...
        {elseif $facet.widgetType == 'dropdown'}
          {block name='facet_item_dropdown'}
            <ul id="facet_{$_expand_id}" class="faceted-dropdown collapse{if !$_collapse} show{/if}">
              <li>
        ...
        {elseif $facet.widgetType == 'slider'}
          {block name='facet_item_slider'}
            {foreach from=$facet.filters item="filter"}
              ...
              >
                <li>
                  <div id="slider-range_{$_expand_id}" class="slider-range"></div>
                  <p id="facet_label_{$_expand_id}" class="text-center font-weight-bold mt-2">{$filter.label}</p>
                </li>
              </ul>
            {/foreach}
          {/block}
