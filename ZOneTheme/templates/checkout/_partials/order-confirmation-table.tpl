...
<div id="order-items" class="col-12 col-md-8">
  ...
  <div class="order-confirmation-table grid-small-padding">
    {block name='order_confirmation_table'}
      ...
      <div class="order-confirmation-total text-right">
        <div class="row">
          {foreach $subtotals as $subtotal}
            {if $subtotal.type !== 'tax' && $subtotal.label !== null && isset($subtotal.value)}
              <div class="col-8"><label>{$subtotal.label}</label></div>
              <div class="col-4">
          ...
          {if $subtotals.tax.label !== null}
            <div class="col-8"><label>{l s='%label%:' sprintf=['%label%' => $subtotals.tax.label] d='Shop.Theme.Global'}</label></div>
            <div class="col-4"><span class="price price-normal">{$subtotals.tax.value}</span></div>
          {/if}

          {if !$configuration.display_prices_tax_incl && $configuration.taxes_enabled}
            <div class="col-8"><label>{$totals.total.label}&nbsp;{$labels.tax_short}</label></div>
            <div class="col-4"><span class="price">{$totals.total.value}</span></div>
            <div class="col-8"><label>{$totals.total_including_tax.label}</label></div>
            <div class="col-4"><span class="price price-total">{$totals.total_including_tax.value}</span></div>
          {else}
            <div class="col-8"><label>{$totals.total.label}&nbsp;{if $configuration.taxes_enabled}{$labels.tax_short}{/if}</label></div>
            <div class="col-4"><span class="price price-total">{$totals.total.value}</span></div>
          {/if}
        </div>
      </div>
    {/block}
